...
function gamesHighlightClass(item) {
  return item.kind === "turn" || item.kind === "choosing" ? " highlight" : "";
}

function gamesRowWaitingClass(item) {
...
function horseGamesRowHTML(item) {
  {
    ...
    if (item.kind === "invite") {
      ...
      <div class="games-row">
        <span class="games-row-icon" aria-hidden="true">🐴</span>
        <span class="games-row-info"><span class="games-row-title">${escapeHtml(item.from)} invited you to Horse${others ? ` vs. ${escapeHtml(others)}` : ""}</span></span>
        <span class="games-row-actions">
...
function finishedGameRowHTML(entry) {
  ...
  if (entry.mode === "horse") {
    ...
    return `<button type="button" class="games-row" data-games-recent-horse="${game.id}">
        <span class="games-row-icon" aria-hidden="true">🐴</span>
...
function renderGamesScreen() {
  ...
  if (!hasOpen && !hasActive) {
    ...
    recentListEl.querySelectorAll(".games-row-avatar").forEach((el) => setAvatarEl(el, el.dataset.avatar));
    return;
  }
  ...
  listEl.querySelectorAll(".games-row-avatar").forEach((el) => setAvatarEl(el, el.dataset.avatar));
}
...
const SHARE_MESSAGES_MY_BONANZA = [
  ...
  (ctx) => `${ctx.dayText} today, ${ctx.weekText} this week 🧾 The bonanza sends an invoice.`,
  (ctx) => `Logged ${ctx.dayText} today. ${ctx.weekText} for the week. My reflection gave me a nod of respect.`,
  (ctx) => `${ctx.weekText} ${ctx.activityWord} this week 🏗️ I am basically a construction project at this point.`,
  (ctx) => `Today: ${ctx.dayText}. Week: ${ctx.weekText}. The couch has stopped waiting for me.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} down today, ${ctx.weekText} on the week. Somewhere a personal trainer is nervous.`,
  (ctx) => `${ctx.weekText} this week, ${ctx.dayText} of it today 📐 My form has its own zip code.`,
  (ctx) => `Put in ${ctx.dayText} today. ${ctx.weekText} for the week. The bathroom scale requested a raise.`,
  (ctx) => `${ctx.dayText} today, ${ctx.weekText} this week 🧠 My brain has started counting reps in its sleep.`,
  (ctx) => `Weekly total: ${ctx.weekText}. Today's cut: ${ctx.dayText}. The bonanza does not believe in rest days, apparently.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} today 🎯 ${ctx.weekText} this week. My shadow is getting jacked too.`,
  (ctx) => `${ctx.weekText} on the week, ${ctx.dayText} freshly added 🧱 Building something, not sure what yet.`,
  (ctx) => `Today's number: ${ctx.dayText}. Weekly number: ${ctx.weekText}. My gym membership feels personally attacked.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} today, ${ctx.weekText} this week 🥵 The floor and I need a mediator.`,
  (ctx) => `${ctx.weekText} this week 📈 ${ctx.dayText} of it happened before I fully woke up.`,
  (ctx) => `Racked up ${ctx.dayText} today, ${ctx.weekText} for the week. My arms have started sending group texts.`,
  (ctx) => `${ctx.dayText} today. ${ctx.weekText} this week. I am contractually obligated to keep going now.`,
  (ctx) => `${ctx.weekText} ${ctx.activityWord} this week, ${ctx.dayText} today alone 🚨 Send help, or don't, I'm having fun.`,
  (ctx) => `Today: ${ctx.dayText}. Week: ${ctx.weekText}. My chiropractor just blocked my number.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} logged 📊 ${ctx.weekText} for the week. The bonanza is undefeated.`,
  (ctx) => `${ctx.weekText} on the books this week, ${ctx.dayText} of it fresh 🔥 My joints have filed a joint statement.`,
  (ctx) => `${ctx.dayText} today, ${ctx.weekText} this week 🫠 I have become a cautionary tale with good form.`,
  (ctx) => `Weekly damage report: ${ctx.weekText}. Today's contribution: ${ctx.dayText}. The floor keeps a diary now.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} today 💼 ${ctx.weekText} this week. This is technically a full-time job now.`,
  (ctx) => `${ctx.weekText} this week, capped off with ${ctx.dayText} today 🧊 My cool-down never actually cools down.`,
  (ctx) => `Today's grind: ${ctx.dayText}. Weekly grind: ${ctx.weekText}. My towel has given up on drying anything.`,
  (ctx) => `${ctx.dayText} today, ${ctx.weekText} this week 🛠️ I am under renovation and slightly over budget.`,
  (ctx) => `${ctx.weekText} ${ctx.activityWord} this week 🐘 ${ctx.dayText} of it today. Never forgetting, apparently, because I keep doing this.`,
  (ctx) => `Logged ${ctx.dayText} today, ${ctx.weekText} this week 🏔️ The summit keeps moving, rudely.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} today. ${ctx.weekText} this week. My playlist has started requesting hazard pay.`,
  (ctx) => `Today: ${ctx.dayText}. This week: ${ctx.weekText}. The bonanza does not RSVP, it just shows up.`,
  (ctx) => `${ctx.weekText} on the week, ${ctx.dayText} today 🧯 My motivation caught fire and I let it.`,
  (ctx) => `${ctx.dayText} ${ctx.activityWord} down 🧮 ${ctx.weekText} this week. My calculator needed a minute.`,
  (ctx) => `Weekly tally sits at ${ctx.weekText}, ${ctx.dayText} freshly baked in 🥧 Nothing about this is a coincidence.`,
  (ctx) => `${ctx.dayText} today, ${ctx.weekText} this week 🪤 The floor keeps setting traps and I keep walking in.`,
  (ctx) => `${ctx.weekText} ${ctx.activityWord} this week, ${ctx.dayText} of it just now 🎬 Directed, produced, and suffered by me.`,
  (ctx) => `Today's count: ${ctx.dayText}. Week's count: ${ctx.weekText}. My arms have opened a formal complaint department.`,
];
...
function renderCardsCompletedStack() {
  ...
  $("cards-completed-count").textContent = formatNumber(cleared.length);
}
...
function advanceToNextCard() {
  ...
  if (state.currentCard) {
    state.cardsCleared.push(state.currentCard);
    renderCardsCompletedStack();
  }
...
function renderChainOfPainCountStage() {
  ...
  $("chainofpain-timer").textContent = chainOfPainCountdown(chainOfPainSegmentDurationMs(chainOfPainState.rules) - (performance.now() - chainOfPainState.segmentStartedAt));
  $("chainofpain-correction-row").classList.toggle("hidden", exercise === "plank");
  $("chainofpain-counter-badge").classList.toggle("hidden", exercise === "plank");
  $("btn-chainofpain-cancel").classList.add("hidden");
...
async function chainOfPainReadyForNextSegment() {
  if (chainOfPainState.stage !== "resting") return;
  chainOfPainState.stage = "loading";
  stopChainOfPainTicker();
