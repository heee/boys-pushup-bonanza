...
test("Sharpshooter targets span 10-25% of a personal best and 3-10 without history", () => {
  assert.equal(sharpshooterTargetForBest(40, () => 0), 4);
  assert.equal(sharpshooterTargetForBest(40, () => 0.999999), 10);
...
test("Cock Mode hides the generic hero/highscore/thermometer for its own pace-duel visualization", () => {
  const hud = workoutHudModel("cock", 40);
  assert.equal(hud.cock, true);
  assert.equal(hud.hideHero, true);
  assert.equal(hud.hideHighscore, true);
  assert.equal(hud.hideThermometer, true);
});
...
test("the player's own personal-best rung folds them in alongside any rivals tied there, independent of the live rung", () => {
  const self = { name: "Dev", avatar: { emoji: "🐯" }, rung: 7 };
  const rows = ladderRungRows(6, [{ rung: 7, names: ["Mia"], users: [{ name: "Mia" }] }], () => false, self);
  const selfRow = rows.find((row) => row.rung === 7);
  assert.deepEqual(selfRow.rival.users, [{ name: "Mia" }, { ...self, self: true }]);
  const active = rows.find((row) => row.rung === 6);
  assert.equal(active.rival, null);
});
